...
class UIWindow
{
	...
	void append_overview_row(const std::string &basename, const std::string &fullpath);
	...
	void send_input(const std::string &entry_name, const std::string &wrapper_name, asio::ip::tcp::socket *const *const socket);
	void send_input_gdb();
	void send_input_trgt();
	void stop_all();
	void toggle_all(const std::string &box_name);
	void toggle_all_gdb();
	...
	void create_mark(Gtk::TextIter &iter, Glib::RefPtr<Gsv::Buffer> &source_buffer, const std::string &fullpath);
	...
	void on_line_mark_clicked(Gtk::TextIter &iter, GdkEvent *const event, const std::string &fullpath);
	void send_data_to_active(asio::ip::tcp::socket *const *const socket, const std::string &data);
	...
	void color_overview();
	void update_overview(const int rank, const std::string &fullpath, const int line);
	void set_position(const int rank, const std::string &fullpath, const int line);
	void open_file();
...
public:
	...
	bool send_data(asio::ip::tcp::socket *const socket, const std::string &data);
	...
	inline void set_conns_gdb(const int rank, asio::ip::tcp::socket *const socket)
	{
	...
	inline void set_conns_trgt(const int rank, asio::ip::tcp::socket *const socket)
	{
