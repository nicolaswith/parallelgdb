...
*/
static inline int EndOfStr(const char *s)
{
	if (*s == '"')
	{
		s++;
...
/* What's a valid variable name?
   I'll assume a-zA-Z0-9_- */
inline int mi_is_var_name_char(char c)
{
...
int mi_res_simple(mi_h *h, int tclass)
{
	mi_output *r, *res;
	int ret = 0;
...
{
	return mi_res_simple(h, MI_CL_DONE);
}

int mi_res_simple_exit(mi_h *h)
{
	return mi_res_simple(h, MI_CL_EXIT);
}

int mi_res_simple_running(mi_h *h)
{
	return mi_res_simple(h, MI_CL_RUNNING);
}

int mi_res_simple_connected(mi_h *h)
{
	return mi_res_simple(h, MI_CL_CONNECTED);
}
	...
	if (res)
	{
		while (c)
		{
			if (c->type == t_const)
			{
				...
				else if (strcmp(c->var, "fullname") == 0)
				{
					res->fullname = c->v.cstr;
					c->v.cstr = NULL;
				}
...
static enum mi_stop_reason reason_values[] = {
	sr_bkpt_hit,
